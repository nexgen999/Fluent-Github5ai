...
import { useConfig, type HubSource } from '../contexts/ConfigContext';
...
export default function FileBrowser() {
  ...
  // Liste des sources disponibles (par défaut + sources additionnelles)
  const sources = useMemo(() => {
    if (!config) return [];
    
    const defaultSource: HubSource = {
      id: 'default',
      name: 'Principal',
  ...
  // Initialiser la première source
  useEffect(() => {
  ...
  }, [currentPath, config, activeSourceId]);

  const loadFiles = async (path: string) => {
    if (!config || !activeSource) return;
    setLoading(true);
    try {
      const data = await fetchFiles(path, config, activeSource);
      setFiles(data);
    } catch (err) {
    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <header className="p-6 border-b border-dim-border flex items-center justify-between shrink-0">
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold text-xl">{activeSource?.name || 'files'}</span>
          {currentPath && (
      ...
      {/* Sources Tabs */}
      {sources.length > 1 && (
        <div className="px-6 py-2 bg-[#1e2732]/20 border-b border-dim-border flex gap-4 overflow-x-auto custom-scrollbar no-scrollbar">
          {sources.map(source => (
            <button
              key={source.id}
              onClick={() => handleSourceChange(source.id)}
              className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-wider transition-all border whitespace-nowrap ${
                activeSourceId === source.id 
                  ? "bg-dim-accent border-dim-accent text-white shadow-lg shadow-dim-accent/20" 
                  : "bg-transparent border-dim-border text-dim-muted hover:border-dim-muted"
              }`}
            >
              <Database size={10} />
              {source.name}
